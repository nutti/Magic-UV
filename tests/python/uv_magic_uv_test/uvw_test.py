...
class TestUVWBox(common.TestBase):
    ...
    idname = [
        # UVW
        ('OPERATOR', 'uv.muv_uvw_operator_box_map'),
    ]
    ...
    def test_ng_no_uv(self):
        ...
        result = bpy.ops.uv.muv_uvw_operator_box_map(assign_uvmap=False)
        self.assertSetEqual(result, {'CANCELLED'})

    def test_ok_default(self):
        print("[TEST] (OK) Default")
        bpy.ops.mesh.select_all(action='SELECT')
        result = bpy.ops.uv.muv_uvw_operator_box_map()
...
class TestUVWBestPlaner(common.TestBase):
    ...
    idname = [
        # UVW
        ('OPERATOR', 'uv.muv_uvw_operator_best_planer_map'),
    ]
    ...
    def test_ng_no_uv(self):
        ...
        print("[TEST] (NG) No UV")
        bpy.ops.mesh.select_all(action='SELECT')
        result = bpy.ops.uv.muv_uvw_operator_best_planer_map(assign_uvmap=False)
        self.assertSetEqual(result, {'CANCELLED'})

    def test_ok_default(self):
        print("[TEST] (OK) Default")
        bpy.ops.mesh.select_all(action='SELECT')
        result = bpy.ops.uv.muv_uvw_operator_best_planer_map()
        self.assertSetEqual(result, {'FINISHED'})
