...
class TestFlipRotateUV(common.TestBase):
    ...
    idname = [
        # Flip/Rotate UVs
        ('OPERATOR', 'uv.muv_flip_rotate_uv_operator'),
    ]
    ...
    def test_ng_no_uv(self):
        ...
        print("[TEST] (NG) No UV")
        result = bpy.ops.uv.muv_flip_rotate_uv_operator()
    ...
    def test_ok_user_specified(self):
        ...
        bpy.ops.mesh.uv_texture_add()
        bpy.ops.mesh.select_all(action='SELECT')
        result = bpy.ops.uv.muv_flip_rotate_uv_operator(flip=True, rotate=5, seams=False)
        self.assertSetEqual(result, {'FINISHED'})
